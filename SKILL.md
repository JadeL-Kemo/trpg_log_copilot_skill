...
| **3** | `03_时间线.md` | `time`(HH:MM标签) `event` `event_date`=`YYYY-MM-DD`+后缀 `-{NNN}`(零填充) `category` `scene_id` `participants` `related_clues` `notes` | event_date 禁止"当日""现在"·后缀用010/020/030留插入空间 |
